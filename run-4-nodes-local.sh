...
# Run 4 lean nodes locally without proof generation

set -e

echo "Starting 4 local lean nodes..."
echo ""
...
if [ ! -d "config" ] || [ ! -f "config/validator_registry.yml" ]; then
  echo "Error: Config directory not found"
  echo "Please run './setup-local-devnet.sh' first to generate keys"
  exit 1
fi

# Check if genesis config exists
if [ ! -f "genesis/config.yaml" ]; then
  echo "Error: Genesis config not found"
  echo "Please run './setup-genesis.sh' first to generate shared genesis"
  exit 1
fi
...
# Start node0 (bootnode)
echo "Starting node0 (bootnode)..."
$REAM_BIN \
  --ephemeral \
  lean_node \
...
# Wait for node0 to start and extract peer ID
echo "Waiting for node0 to start and generate peer ID..."
...
echo ""
echo "Logs:"
echo "  tail -f node0.log  # Watch node0"
echo "  tail -f node1.log  # Watch node1"
echo "  tail -f node*.log  # Watch all nodes"
echo ""
echo "Stop all nodes:"
echo "  ./stop-local-nodes.sh"
echo "  OR: pkill -f 'ream.*lean_node'"
echo ""
echo "APIs:"
